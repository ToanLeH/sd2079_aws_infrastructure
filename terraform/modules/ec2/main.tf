...
#   filter {
#     name   = "image-id"
#     values = [lookup(each.value, "ami", "ami-0109aae92ef039cb2")]
#   }

#   owners = [lookup(each.value, "ami-owner", "663535708029")]
# }

data "aws_ami" "info" {
  filter {
    name   = "image-id"
    values = ["ami-0944b82348d881a72"]
  }
...
resource "aws_instance" "default" {
  ...
  instance_type               = lookup(each.value, "instance-type", "t3a.large")
