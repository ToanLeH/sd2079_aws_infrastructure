...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    ...
    //try to comment this out to allow all public access to EKS API Endpoint
...
# data "aws_iam_policy" "eks_worknode_ebs_policy" {
#   name = "Amazon_EBS_CSI_Driver"
# }

# And attach the new policy
# resource "aws_iam_role_policy_attachment" "worknode-AmazonEBSCSIDriver" {
#   policy_arn = data.aws_iam_policy.eks_worknode_ebs_policy.arn
...
# data "aws_iam_policy_document" "identity_provider_assume_role_policy" {
#   statement {
#     sid     = "EKSClusterAssumeRole"
#     actions = [
#       "sts:AssumeRole",
...
#  assume_role_policy = data.aws_iam_policy_document.identity_provider_assume_role_policy.json
#  name = "aws-node"
# }

# resource "aws_iam_role" "aws_node" {
#   name = "identity-provider-${aws_eks_cluster.this[0].name}-aws-node"
#   assume_role_policy =  templatefile("oidc_assume_role_policy.json", { 
#     OIDC_ARN = aws_iam_openid_connect_provider.oidc_provider[0].arn, 
#     OIDC_URL = replace(aws_iam_openid_connect_provider.oidc_provider[0].url, "https://", ""), 
...
# resource "aws_iam_policy" "identity_provider_role_policy" {
#   name        = "identity-provider-${aws_eks_cluster.this[0].name}"
#   path        = "/"
#   description = "Policy for oidc"

#   policy = data.aws_iam_policy_document.identity_provider_assume_role_policy.json
# }

# resource "aws_iam_role_policy_attachment" "identity-provider-role-attachment" {
#   policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"
#   role       = aws_iam_role.this[0].name
# }

# resource "aws_iam_role_policy_attachment" "identity-provider-role-attachment" {
#   policy_arn = aws_iam_policy.identity_provider_role_policy.arn
#   role       = aws_iam_role.this[0].name
# }
...
#   cluster_name = aws_eks_cluster.this[0].name
#   oidc {
#     client_id = "${substr(aws_eks_cluster.this[0].identity.0.oidc.0.issuer, -32, -1)}"
#     identity_provider_config_name = "sdemonew"
#     issuer_url = "https://${aws_iam_openid_connect_provider.oidc_provider[0].url}"
#   }
# }
...
module "ebs_csi_controller_role" {
  ...
  version = "5.11.1"
  create_role = true
  role_name = "${aws_eks_cluster.this[0].name}-ebs-csi-controller"
  provider_url = replace(aws_iam_openid_connect_provider.oidc_provider[0].url, "https://", "")
...
#   #service_account_role_arn = local.cluster_role
...
#   name        = "ebs-iam-policy-${var.cluster_name}"
#   path        = "/"
#   description = "Policy forEBS"

#   policy = data.aws_iam_policy_document.ebs_assume_role_policy.json
# }

# resource "aws_iam_role_policy_attachment" "ebs_role_policy_attachment" {
#   policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"
...
resource "aws_eks_addon" "ebs_addon" {
  ...
  resolve_conflicts        = "OVERWRITE"
  service_account_role_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${aws_eks_cluster.this[0].name}-ebs-csi-controller" 
